...
int32_t main(int32_t argc, char *argv[]) {
	...
	for (int32_t i = 2; i < argc; i++) {
		...
		char action[42], filename[128], httpname[128];
		int32_t fd = 0;
		size_t file_size = -1;
		...
		strncpy(action, argv[i], ptr - argv[i]); // obatin action

		if ((strcmp(action, "r") != 0) && (strcmp(action, "s") != 0) && (strcmp(action, "a") != 0)) {
			fprintf(stderr, "HTTP/1.1 400 Bad Request\n");
			continue;
		}
		...
		// obtain httpname(new_name) and filename(existing_name)
		if (strcmp(action, "r") == 0) {
			if(buf[0] == '/'){
				strncpy(httpname, buf + 1, ptr - buf - 1);
				//ptr += 1;
			}else{
				strncpy(httpname, buf, ptr - buf);
			}
			if(buf[ptr+1-buf] == '/'){
				strncpy(filename, ptr + 2, strlen(buf) - (ptr - buf) - 2);
			}else{
				strncpy(filename, ptr + 1, strlen(buf) - (ptr - buf) - 1);
			}
		} else {
			if(buf[0] == '/'){
				strncpy(filename, buf + 1, ptr - buf - 1);
				//ptr += 1;
			}else{
				strncpy(filename, buf, ptr - buf);
			}
			if(buf[ptr+1-buf] == '/'){
				strncpy(httpname, ptr + 2, strlen(buf) - (ptr - buf) - 2);
			}else{
				strncpy(httpname, ptr + 1, strlen(buf) - (ptr - buf) - 1);
			}
		}
		if(strlen(httpname) == 0 || strlen(filename) == 0){
			fprintf(stderr,  "HTTP/1.1 400 Bad Request\n");
			continue;
		}
		memset(buf, 0, BUFFER_SIZE);
			...
			if (send(sock, buf, strlen(buf), 0) == -1) {
				...
				if ((recv_result = recv(sock, buf, sizeof(buf) - 1, MSG_WAITALL)) == -1) {
					...
					if ((fd = open(filename, O_WRONLY | O_TRUNC)) == -1) {
						if ((fd = open(filename, O_CREAT | O_WRONLY | O_TRUNC, S_IRWXU | S_IRWXG | S_IRWXO)) == -1) {
							fprintf(stderr, "%s\n", strerror(errno));
							close(fd);
							close(sock);
							continue;
		...
		// a -> ALIASES command
			...
			// a:existing_name(filename):new_name(httpname)
			// PATCH filename new_name\r\n\r\n
			// ALIAS filename new_name\r\n
			strcat(buf, "PATCH ");
			strcat(buf, filename);
			strcat(buf, " HTTP/1.1\r\n\r\nALIAS ");
			strcat(buf, filename);
