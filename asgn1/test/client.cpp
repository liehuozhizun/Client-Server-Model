 ...
 * This program implements the basic functions of a HTTP client,
 * which can communicate with an HTTP server. Sending requests 
 * to and receiving responses from the HTTP server.
 * 
 */
...
int32_t main(int32_t argc, char *argv[]){
	char file_buf[BUFFER_SIZE], buf[BUFFER_SIZE];
	char address[33];
	int32_t port_number;
	char * ptr, * ptr_tem, * ptr_content_length;
	
	if(argc == 1){
		fprintf(stderr, "%s\n", "SET UP FAILED: no valid address and port number");
		exit(EXIT_FAILURE);
	}
	...
	for(int32_t i = 2; i < argc; i++){
		...
		char action[42]; memset(action, 0, sizeof(action));
		char filename[42]; memset(filename, 0, sizeof(action));
		char httpname[42]; memset(httpname, 0, sizeof(action));
		int32_t fd = 0, cl, file_size = -1;
			...
			if((fd = open(filename, O_RDONLY)) == -1){
				...
				if((read_result = read(fd, file_buf, sizeof(file_buf))) == -1){
					...
					if(send(sock, buf, strlen(buf), 0) == -1){
						...
						// receive response
						if((recv_result = recv(sock, buf, sizeof(buf) - 1, 0)) == -1){
							fprintf(stderr, "%s\n", strerror(errno));
						}else{
							if((ptr = strstr(buf, "\r\n\r\n")) == NULL){
								fprintf(stderr, "%s\n", "Wrong response format, lacking of '\\r\\n\\r\\n'");
								close(sock);
								close(fd);
								continue;
							}else{
						...
						// send the rest of file_buf data
						do{
		...
		// r -> GET request
			...
			if(send(sock, buf, strlen(buf), 0) == -1){
				...
				if((recv_result = recv(sock, buf, sizeof(buf) -1, MSG_WAITALL)) == -1){
					...
					ptr = strstr(buf, "\r\n");
					if((ptr_tem = strstr(buf, "\r\n\r\n")) == NULL){
						fprintf(stderr, "%s\n", "Wrong response format, lacking of '\\r\\n\\r\\n'");
						close(sock);
						continue;
					}
					strncpy(file_buf, buf, strlen(buf) - 4); // status code stored in file_buf temporarily
					if((strstr(file_buf, "200 OK") == NULL) && (strstr(file_buf, "201 Created") == NULL)){
						fprintf(stderr, "%s\n", file_buf);
						close(sock);
						continue;
					}					
					...
					if((ptr_content_length = strstr(buf, "Content-Length: ")) > ptr_tem){
						strncpy(file_buf, ptr_tem + 4, strlen(ptr_tem + 4));
					}else{
						if(ptr_content_length != NULL){
					...
					do{
						...
						if(file_size == 0){ // use Content-Length to identify end of file_buf
							break;
						}
						memset(file_buf, 0, BUFFER_SIZE);
