...
int32_t main(int32_t argc, char *argv[]) {
  ...
  for (int32_t i = 2; i < argc; i++) {
    ...
    char filename[42];
    memset(filename, 0, sizeof(action));
    char httpname[42];
    memset(httpname, 0, sizeof(action));
    ...
    // r -> GET request
      ...
      if (send(sock, buf, strlen(buf), 0) == -1) {
        ...
        if ((recv_result = recv(sock, buf, sizeof(buf) - 1, MSG_WAITALL)) ==
            -1) {
          fprintf(stderr, "%s\n", strerror(errno));
        } else {
          ...
          if ((ptr_content_length = strstr(buf, "Content-Length: ")) >
              ptr_tem) {
